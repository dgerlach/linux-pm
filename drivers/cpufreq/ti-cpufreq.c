...
static int ti_cpufreq_get_efuse(u32 *efuse_value)
{
	...
	ret = of_property_read_u32(np, "ti,efuse-mask", &efuse_mask);
	if (ret)
		efuse_mask = 0xffffffff;

	ret = of_property_read_u32(np, "ti,efuse-shift", &efuse_shift);
	if (ret)
		efuse_shift = 0;
...
MODULE_DESCRIPTION("Texas Instruments CPUFreq/OPP driver");

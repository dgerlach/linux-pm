...
/**
 * dev_pm_opp_get_min_voltage() - Gets the min voltage corresponding to an opp
 * @opp:	opp for which minimum voltage has to be returned for
 *
 * Return: minimum voltage in micro volt corresponding to the opp, else
 * return 0
 *
 * Locking: This function must be called under rcu_read_lock(). opp is a rcu
 * protected pointer. This means that opp which could have been fetched by
 ...
 * pointer.
 */
unsigned long dev_pm_opp_get_min_voltage(struct dev_pm_opp *opp)
{
	struct dev_pm_opp *tmp_opp;
	unsigned long v = 0;
...
EXPORT_SYMBOL_GPL(dev_pm_opp_get_min_voltage);

/**
 * dev_pm_opp_get_max_voltage() - Gets the max voltage corresponding to an opp
 * @opp:	opp for which maximum voltage has to be returned for
 *
 * Return: maximum voltage in micro volt corresponding to the opp, else
 * return 0
 *
 * Locking: This function must be called under rcu_read_lock(). opp is a rcu
 * protected pointer. This means that opp which could have been fetched by
 ...
 * pointer.
 */
unsigned long dev_pm_opp_get_max_voltage(struct dev_pm_opp *opp)
{
	struct dev_pm_opp *tmp_opp;
	unsigned long v = 0;
...
EXPORT_SYMBOL_GPL(dev_pm_opp_get_max_voltage);
...
{
	int ret;
	ret = dev_pm_opp_domain_opp_supported_by_supply(opp_table->opp_domain,
							opp->u_volt_min,
							opp->u_volt_max);
	if (!ret)

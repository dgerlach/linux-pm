...
/**
 * dev_pm_opp_domain_set_rate() - Transition voltage a frequency for an opp domain
 ...
 * is unbound.
 */
struct pm_opp_domain_dev *devm_opp_domain_register(struct device *dev,
...
void devm_opp_domain_unregister(struct pm_opp_domain_dev *oppdm_dev)
{
	...
	rc = devres_release(dev, devm_opp_domain_release, devm_oppdm_dev_match, oppdm_dev);
	if (rc != 0)
		WARN_ON(rc);
}

				   ...
				   u32 dsp_addr)
{
	...
	if (!status) {
		 ...
		 */
		mbox_user.txcb = NULL;
		mbox_user.rxcb = io_mbox_msg;
		mbox_user.cl_id = dev_context;
		mbox_user.tx_block = false;
		mbox_user.tx_tout = 0;
		mbox_user.link_data = NULL;
		mbox_user.knows_txdone = false;
		sprintf(mbox_user_name, "omap2:dsp");
		mbox_user.chan_name = mbox_user_name;
		dev_context->mbox = ipc_request_channel(&mbox_user);
		if (!dev_context->mbox) {
...
static int bridge_brd_stop(struct bridge_dev_context *dev_ctxt)
{
	...
	if (dev_context->mbox) {
		...
		ipc_free_channel(dev_context->mbox);

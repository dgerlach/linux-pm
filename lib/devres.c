...
/**
 * devm_ioremap_exec_wc - Managed ioremap_exec_wc()
 * @dev: Generic device to remap IO address for
 * @offset: BUS offset to map
 * @size: Size of map
 *
 * Managed ioremap_exec_wc().  Map is automatically unmapped on driver
 * detach.
 */
/*
void __iomem *devm_ioremap_exec_wc(struct device *dev,
				   resource_size_t offset,
				   unsigned long size)
{
	void __iomem **ptr, *addr;
	...
	addr = ioremap_exec_wc(offset, size);
	if (addr) {
		*ptr = addr;
		devres_add(dev, ptr);
	} else
